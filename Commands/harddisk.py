    ...
    async def harddisk(self, ctx):
        if self.guild.get_whitelist(ctx.guild.id) == ctx.channel.id or self.guild.get_whitelist(ctx.guild.id) == 0:
            if self.check.guild(ctx.guild.id) is False:
                return await ctx.send(embed=discord.Embed(description=f"Me parece que seu servidor não está configurado na minha database. COMMAND: @{self.client.user.name}!?", color=0xef0027))
            ...
            return await ctx.send(embed=discord.Embed(description=f"{ctx.author.mention}, você não pode executar comandos nesse chat.", color=0xef0027))
